...
### Vai trò trong buổi workshop

Tôi thực hiện bài này với vai trò **người dùng thật kiêm product analyst**: vừa trực tiếp dùng thử V-AI để trải nghiệm sản phẩm, vừa phân tích hành vi của AI theo góc nhìn product — tìm điểm gãy trong workflow, đặt tên lỗi theo layer, và viết finding thành quyết định cụ thể.

### Việc đã làm

1. **Chọn sản phẩm:** Chọn V-App — V-AI vì đây là app tôi đang dùng thực tế, dễ kiểm chứng và có thể tự thu thập evidence nhanh.
2. **Dùng thử & ghi evidence:** Thử 2 lượt chat liên tiếp với prompt có chủ đích — câu đầu hỏi quan hệ giữa hai nhân vật, câu sau cố tình đặt tiền đề sai để xem bot có phát hiện ra không.
3. **Chụp screenshot:** Ghi lại toàn bộ response của bot làm bằng chứng.
4. **Phân tích 4 paths:** Đánh giá từng path (Happy / Low-confidence / Failure / Correction) dựa trên hành vi quan sát thực tế.
5. **Viết 3 findings:** Mỗi finding theo đúng format: trigger → failure → impact → layer lỗi → hướng sửa + test case.
6. **Vẽ sketch as-is / to-be:** Dùng ASCII flow để thể hiện luồng hiện tại (có đánh dấu điểm gãy) và luồng đề xuất (có đánh dấu điểm sửa).
7. **Viết SPEC Change:** Chuyển finding thành Acceptance Criteria cụ thể để team engineering có thể implement.


### Bài học sau khi làm bài

1. **"Bot ngu" không phải finding.** Cùng một hành vi sai, nếu viết thành "AI trả lời sai" thì vô nghĩa; nếu viết thành "Intent không được parse → tiền đề sai không bị phủ nhận → user hiểu nhầm" thì có thể action ngay.

2. **Low-confidence path quan trọng hơn tôi nghĩ.** Trước khi làm bài, tôi nghĩ vấn đề chính là bot trả lời sai. Sau khi phân tích, tôi thấy vấn đề gốc rễ là bot **không bao giờ thể hiện sự không chắc chắn** — dẫn đến tất cả các failure sau đó.

3. **Một sản phẩm không có Correction path = user không có cách thoát.** Khi bot sai mà user không có chỗ sửa, user chỉ còn cách thoát app. Đây là churn ẩn mà log thông thường không capture được.

4. **Prompt "xoắn" là công cụ needfinding hiệu quả.** Việc cố tình đặt tiền đề sai giúp reveal ngay failure mode mà trong điều kiện bình thường có thể không gặp — đây là kỹ thuật mà PM/QA nên thực hành thường xuyên với AI feature.
